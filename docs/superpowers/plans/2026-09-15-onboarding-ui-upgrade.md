...
- **`tsc --noEmit` must pass at every commit.** `next.config.mjs` sets `typescript.ignoreBuildErrors: true`, so `pnpm build` passing proves nothing about type correctness.
...
// ... 89 more, one per unique icon name ...
...
Expected: `generate-icon-data: wrote 91 icons.` then `verify-icon-map: all 93 icons resolved.`

(91 unique targets for 93 Lucide names — `CheckCircle`/`CheckCircle2` and `Send`/`SendIcon` and `Paperclip`/`PaperclipIcon` collapse onto shared names.)
